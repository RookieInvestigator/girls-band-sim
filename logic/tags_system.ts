...
export const TAG_MODIFIERS: Record<string, { 
    ...
    // === LEGENDARY / UR TAGS ===
    '作词天才': { growthMult: 1.2, statsBonus: { lyrics: 2.0, mental: -0.2 }, stressMult: 1.2 }, 
    '绝对王者': { growthMult: 1.4, money: -200, stressMult: 1.6, successRate: 10 }, 
    '双重人格': { growthMult: 1.1, statsBonus: { stagePresence: 1.5, technique: 1.0, mental: -0.5 } }, 
    '领袖气质': { growthMult: 1.2, statsBonus: { affection: 1.0 }, stressMult: 0.8 }, 
    '铁人': { fatigueMult: 0.4, growthMult: 1.1 }, 
    '天然呆': { successRate: -10, growthMult: 1.5, stressMult: 0.5 }, 
    '自由人': { growthMult: 1.4, statsBonus: { creativity: 2.0 }, successRate: -5, stressMult: 0.8 }, 
    '凡人拟态': { growthMult: 1.3, successRate: 15, statsBonus: { technique: 0.5, arrangement: 0.5 } }, 
    '叛逆者': { statsBonus: { technique: 0.5, stagePresence: 0.3 }, stressMult: 0.8, money: -100 }, 

    // === NEW UR TAGS ===
    '重力': { stressMult: 1.4, statsBonus: { lyrics: 3.0, mental: -0.8, affection: 0.5 }, growthMult: 1.2 }, // Replaced '迷子': Intense emotions (Lyrics++), heavy mental load
    '白月光': { statsBonus: { affection: 2.0, stagePresence: 0.5 }, stressMult: 1.5, fatigueMult: 0.8 }, // White Sakiko: Everyone loves her, but she hides stress
    '摇滚魂': { statsBonus: { stagePresence: 1.0, design: 0.5, technique: -0.2 }, growthMult: 1.2 }, // Riina: Passion > Skill
    '孤狼': { statsBonus: { technique: 1.2, arrangement: 0.5, affection: -2.0 }, stressMult: 1.2 }, // Shiho: Skilled but isolated
    '归国子女': { statsBonus: { composing: 1.0, lyrics: 1.0 }, stressMult: 1.1, successRate: 10 }, // Mia: Skilled but slight culture shock stress
    '天使': { statsBonus: { affection: 2.0, mental: 1.0 }, stressMult: 0.7 }, // Nijika: Heals the team

    // === TRANSFORMED TAGS (BLACK SAKIKO) ===
    '复仇者': { growthMult: 1.5, stressMult: 0.5, statsBonus: { stagePresence: 1.0, affection: -2.0 }, money: 2000 }, 
    '假面': { stressMult: 0.2, statsBonus: { mental: 1.0 }, fatigueMult: 1.5 }, 
    ...
    '街头': { fans: 40, statsBonus: { stagePresence: 0.4 }, money: -50 }, 
    ...
    '玻璃心': { stressMult: 1.8, growthMult: 0.8, statsBonus: { lyrics: 0.5 } }, 
    '乐天派': { stressMult: 0.6, statsBonus: { mental: 0.3 } }, 
    // '重力' moved to UR section
    '社恐': { stressMult: 1.5, growthMult: 1.2, fans: -5 }, 
    '完美主义': { growthMult: 1.1, stressMult: 1.6, successRate: 5 },
    '体力怪物': { fatigueMult: 0.5, statsBonus: { stagePresence: 0.2 } },
    '大小姐': { money: -200, statsBonus: { design: 0.5 } }, 
    '三无': { stressMult: 0.4, statsBonus: { mental: 0.5 }, fans: 5 }, 
...
export const TAG_BAND_MODIFIERS: Record<string, Partial<BandStats>> = {
    ...
    '天然呆': { adaptation: 0.8, interaction: 1.3 }, 
    '自由人': { adaptation: 1.5, precision: 0.8 }, 
    '凡人拟态': { synergy: 1.4, precision: 1.3, tone: 1.1 }, 
    '叛逆者': { dynamics: 1.4, tone: 1.2, synergy: 0.9 }, 

    // === NEW UR TAGS ===
    '重力': { narrative: 1.7, connection: 1.2, stability: 0.6, topic: 1.2 }, // Replaced '迷子': Heavy narrative, high drama/connection but low stability
    '白月光': { synergy: 1.5, aura: 1.2, topic: 1.2 }, // Everyone gathers around her
    '摇滚魂': { aura: 1.3, dynamics: 1.3, precision: 0.8 }, // Pure passion
    '孤狼': { precision: 1.4, technique: 1.3, connection: 0.6 }, // High skill, low bond
    '归国子女': { melody: 1.4, detail: 1.3, connection: 0.9 }, // High quality songs, slightly disjointed
    '天使': { synergy: 1.5, connection: 1.5, stability: 1.4 }, // The glue of the band

    // === TRANSFORMED TAGS (BLACK SAKIKO) ===
    '复仇者': { aura: 1.6, dynamics: 1.3, synergy: 0.5, connection: 0.1 }, 
    '假面': { visual: 1.5, topic: 1.3, interaction: 0.5 }, 
