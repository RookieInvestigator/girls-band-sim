...
export const calculateInteractionOutcome = (member: Member, type: InteractionType): InteractionOutcome => {
  ...
  // FIXED: Changed 'fatigueChange' to 'fatigue' and 'techniqueChange' to 'technique' to match Impact interface in types.ts
  let impact: any = { stressChange: 0, fatigue: 0, affectionChange: 0, technique: 0 };
  ...
  switch (type) {
    case InteractionType.IntensivePractice:
      impact = { stressChange: 12, fatigue: 20, affectionChange: -4 * mult, technique: 2 * mult };
      break;
    case InteractionType.CafeDate:
      // Buff: Fatigue reduction -15 -> -20
      impact = { stressChange: -35 * mult, fatigue: -20 * mult, affectionChange: 12 * mult, technique: 0 };
      break;
    case InteractionType.DeepTalk:
      impact = { stressChange: -20 * mult, fatigue: 4, affectionChange: 8 * mult, technique: 0.5 * mult };
      break;
    case InteractionType.Gift:
      impact = { stressChange: -8 * mult, fatigue: 0, affectionChange: 15 * mult, technique: 0 };
      break;
    case InteractionType.Reprimand:
      impact = { stressChange: 15, fatigue: 0, affectionChange: -8 * mult, technique: 1.5 * mult };
      break;
  }
