    ...
    return (
        <div className="fixed inset-0 bg-slate-950 z-[100] text-white flex flex-col font-sans overflow-hidden select-none">
             ...
             <div className="flex-1 relative z-10 flex flex-col items-center justify-center p-4">
                 ...
                 {/* Live Monitor Box */}
                 <div className="relative w-full max-w-md bg-slate-900/60 backdrop-blur-md border-y-2 border-rose-500/30 p-6 mb-8 overflow-hidden group">
                     ...
                     <div className="flex items-start gap-4">
                         <Monitor size={20} className="text-rose-400 mt-1 shrink-0 animate-pulse"/>
                         <div className="flex-1">
                             <div className="text-[10px] font-black uppercase tracking-[0.2em] text-rose-400 mb-1">当前阶段</div>
                             <div className="text-2xl font-black italic text-white tracking-wide uppercase leading-none pr-1">{phaseLabel}</div>
                             <div className="mt-3 text-xs font-medium text-slate-300 leading-relaxed border-l-2 border-slate-600 pl-3">
                                 {phaseDesc}
                             </div>
                         </div>
                     </div>
                 ...
                 <div className="h-32 flex flex-col items-center justify-center">
                    {lastResult ? (
                        <div key={logs.length} className="animate-in zoom-in slide-in-from-bottom-4 duration-300 text-center">
                            <div className={`text-6xl md:text-7xl font-black italic tracking-tighter uppercase drop-shadow-[0_0_30px_rgba(255,255,255,0.3)] pr-2 ${
                            ...
                            <div className="flex items-center justify-center gap-3 mt-2">
                                <span className="text-2xl font-black text-white tracking-widest bg-black/30 px-3 py-1 rounded">
                                    {lastResult.voltage > 0 ? '+' : ''}{lastResult.voltage} V
                                </span>
                                {lastResult.hypeDelta !== 0 && (
                                    <span className={`text-sm font-bold ${lastResult.hypeDelta > 0 ? 'text-rose-400' : 'text-slate-500'}`}>
                            ...
                            {lastResult.bonuses && lastResult.bonuses.length > 0 && (
                                <div className="flex flex-wrap gap-1 justify-center mt-3 max-w-[280px]">
                                    {lastResult.bonuses.map((b, i) => (
                                        <span key={i} className="text-[9px] font-black uppercase tracking-wider bg-rose-500/20 text-rose-300 px-2 py-1 rounded border border-rose-500/30 animate-pulse">
                                            {b}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className="text-white/10 font-black text-5xl tracking-[0.5em] uppercase animate-pulse">
                            READY
                        </div>
                    )}
                 </div>
             ...
             {/* --- BOTTOM DECK (Cards) --- */}
             <div className="relative z-30 bg-gradient-to-t from-slate-950 via-slate-900 to-transparent pt-10 pb-safe-bottom px-4 lg:px-8">
                 <div className="max-w-7xl mx-auto w-full">
                     ...
                     <div className="flex md:grid md:grid-cols-5 gap-3 h-[160px] md:h-[200px] overflow-x-auto snap-x snap-mandatory scrollbar-hide px-2 md:px-0">
                         {currentOptions.map((opt, i) => {
                             ...
                             return (
                                 <button 
                                    ...
                                    className={`
                                        group relative min-w-[85%] md:min-w-0 w-full h-full rounded-2xl overflow-hidden
                                        bg-slate-900 border-l-[4px] hover:border-l-[8px]
                                        transition-all duration-200 ease-out
                                        hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(0,0,0,0.6)]
                                     ...
                                     <div className="p-4 flex flex-col h-full justify-between relative z-10">
                                         <div>
                                             <div className="flex justify-between items-start mb-2">
                                                <span className={`text-[9px] font-black px-2 py-0.5 rounded border uppercase tracking-wider ${getRiskColor(opt.difficulty)}`}>
                                                    风险 {opt.difficulty}%
                                                </span>
                                                {opt.isSongCard && <Music size={14} className="text-amber-400 animate-pulse"/>}
                                             </div>
                                             
                                             <h4 className="font-black text-sm md:text-lg text-white leading-tight group-hover:text-rose-300 transition-colors line-clamp-2 mb-2 drop-shadow-md tracking-tight">
                                                 {opt.title}
                                             </h4>
                                             <p className="text-[10px] text-slate-400 font-medium leading-relaxed line-clamp-2 opacity-80 group-hover:opacity-100">
                                                 {opt.description}
                                             </p>
                                         </div>
                                         ...
                                         <div className="grid grid-cols-2 gap-2 mt-2">
                                             <div className="bg-black/30 rounded-lg px-2 py-1.5 flex items-center gap-2">
                                                 <Zap size={12} className="text-amber-400 fill-amber-400"/>
                                                 <span className="text-xs font-black text-slate-200">{opt.baseVoltage}</span>
                                             </div>
                                             {opt.hypeGain && (
                                                <div className="bg-black/30 rounded-lg px-2 py-1.5 flex items-center gap-2">
                                                    <TrendingUp size={12} className="text-rose-400"/>
                                                    <span className="text-xs font-black text-slate-200">+{opt.hypeGain}</span>
                                                </div>
                                             )}
                                         </div>
                                     </div>
